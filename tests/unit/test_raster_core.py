...
def test_wp_raster_execution_modes(
    ...
    """
    Verifies that `wp_raster` correctly toggles between Lazy (Dask) and
    Eager (Numpy) execution modes based on the `chunks` argument.
...
def test_merge_raises_incompatible_error(
    ...
    """
    Verifies that `merge_rasters` raises IncompatibleRasterError
