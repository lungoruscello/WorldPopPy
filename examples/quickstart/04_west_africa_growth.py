...
# CRITICAL: resampling='sum' ensures we conserve the population count
# when changing pixel shapes/sizes.
aeqa_africa = "ESRI:102022"  # Africa Albers Equal Area Conic
pop_warped = wp_warp(
    pop_data,
    to_crs=aeqa_africa,
    res=1_000,          # Target resolution in units of 'to_crs' (here: metres)
    resampling='sum'   # Sum population when resampling, do not average it!
)

# 3. Compute Change
pop_change = pop_warped.sel(year=2024) - pop_warped.sel(year=2015)

# 4. Visualise the Raster Data
# vmax=2000 clips the data to highlight the general urban spread.
pop_change.plot(cmap='coolwarm', vmax=2_000, add_colorbar=False)
clean_axis(title="The Abidjan-Lagos Corridor:\nPopulation Boom (2015-2024)")

# 5. Plot Context Information
# We plot only the central countries to avoid double-drawing shared borders.
